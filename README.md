...
매일 7:20에 알림을 보내고, 선택(또는 전자결재 감지)에 따라 출퇴근을 자동 처리합니다.
...
| 시각 | 동작 |
|---|---|
| 07:15 | Windows 작업 스케줄러 → 프로그램 자동 시작 |
| 07:20 | 전자결재 확인 → 연차/반차 감지 시 자동 설정 메시지 전송 (버튼 미발송) |
| 07:20 | 전자결재 미감지 시 텔레그램 버튼 메시지 발송 |
...
**오늘 연차 선택 시**
```
🏖️ 연차로 설정되었습니다. 오늘 출퇴근 체크를 건너뜁니다.
```
...
**증상**: 이미 출근/퇴근했는데도 버튼을 다시 누르려 하거나, 반대로 출근하지 않았는데 이미 출근 처리됨으로 잘못 판단  
**해결**: 근무현황 섹션 텍스트 대신 출근하기/퇴근하기 버튼의 `disabled` 상태로 판단 — 하이웍스가 체크 완료 시 버튼을 비활성화하므로 정확히 동작
