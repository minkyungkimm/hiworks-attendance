...
07:30  텔레그램 알림 발송
  ...
  ├─ 🎉 연차    → "연차 확인" 메시지 전송 / 출퇴근 전체 건너뜀 / 버튼 미발송
  ├─ 🌄 오전반차 → "오전반차 확인" 메시지 전송 / 해당 시간 자동 출퇴근 / 버튼 미발송
  ├─ 🌅 오후반차 → "오후반차 확인" 메시지 전송 / 해당 시간 자동 출퇴근 / 버튼 미발송
  └─ 없음       → 아래 버튼 메시지 발송
...
- 출근 잡(8시, 9시, 오전반차 1시, 오전반차 2시) 건너뜀
- 오후반차 퇴근 잡(12시, 14시) 건너뜀
- 정규/오전반차 퇴근 잡(17시, 18시) 건너뜀
- 18:15 자동 종료는 정상 동작
