...
public class ReminderJob implements Job {
    ...
    public void execute(JobExecutionContext context) throws JobExecutionException {
        ...
        if (config != null) {
            checkApprovalVacation(config, bot);
        }

        // 전자결재 결과와 무관하게 항상 버튼 메시지 발송
        // (결재 없이 연차/반차 사용하는 경우에도 버튼으로 직접 선택 가능)
        log.info("===== 출근 알림 발송 =====");
        if (bot == null) {
    ...
    private void checkApprovalVacation(AppConfig config, TelegramBotService bot) {
